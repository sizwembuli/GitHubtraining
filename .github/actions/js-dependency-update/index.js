...
const setupGit = async () => {
  ...
  await exec.exec('git config --global user.name "gh-automation@email.com"'); 
};
...
async function run() {
    ...
    if (!validateBranchName({ branchName: baseBranch })) {
        core.setFailed('Invalid head-branch name. Branch names should include only characters, numbers, hyphens, underscores and forward slashes.');
        return;
    }
    ...
    if (gitStatus.stdout.length > 0) {
      ...
      logger.debug('Fetching Oktokit API');
